 ...
 * {@link it.polimi.ingsw.server.model.restrictions.ColorRestriction}, respecting only the {@link ValueRestriction} of the
 * personal {@link WindowPatternCard}.
 */
public class IgnoreColorRestrictionEffect extends MoveWithRestrictionsEffect {

    /**
     * This method moves the die ignoring {@link it.polimi.ingsw.server.model.restrictions.ColorRestriction}s.
    ...
    /**
     * This method deletes all the {@link it.polimi.ingsw.server.model.restrictions.ColorRestriction}s in the original
     * glass window.
    ...
    private void deleteColorRestriction(Cell[][] gw) {
        ...
        for(int i=0; i<WindowPatternCard.MAX_ROW; i++) {
            for (int j = 0; j < WindowPatternCard.MAX_COL; j++) {
                ...
                if (!gw[i][j].isEmpty()) {
                    IRestriction valueRestriction = new ValueRestriction(gw[i][j].getContainedDie().getActualDieValue());
                    ...
                    if (restrictionParameter != 0) {
                        IRestriction valueRestriction = new ValueRestriction(restrictionParameter);
                        temporaryRuleSet.add(valueRestriction);
                    }
                }
